...
export default function App() {
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.separator} />
      <Button onPress={stopBackgroundUpdate} title="Stop in foreground" color="red" />
    </View>
  );
}
...
TaskManager.defineTask(LOCATION_TASK_NAME, ({ data, error }) => {
  ...
  if (data) {
    ...
    console.log('Tracking location in background', location.coords);
  }
});
...
const Home = () => {
  ...
  //Location tracking in forground
  const startForegroundUpdate = async () => {
    const { granted } = await Location.getForegroundPermissionsAsync();
    if (!granted) return console.log('Location tracking denied');
